...
//=================================================================
 ...
 //=======================================================================               
                ...
                float3 Diffuse = lerp(BaseColor,_ShadowColor * BaseColor,ramp);
//==============================================================
                float3 Specular = 0;
...
//==========================================================================
                ...
                FinalColor =(Diffuse+ Specular + Emission) * staticShadow + Fresnel;
                //FinalColor =Emission ;
        ...
        Pass{
            ...
            float4 vert(float4 vertex : POSITION, float3 normal : NORMAL) : SV_POSITION
            {
                ...
                VertexPositionInputs positionInputs = GetVertexPositionInputs(vertex.xyz);
                ...
                float3 viewNormal = TransformWorldToView(worldNormal.xyz);
                float3 ndcNormal = normalize(mul((float3x3)UNITY_MATRIX_P,viewNormal).xyz) * clipPos.w;
                clipPos.xy += 0.01 * _Outline * ndcNormal.xy;
                return clipPos;
            }
            half4 frag() : SV_Target
            {
                half4 col = half4(_OutColor.rgb, 1);
                return col;
            }
            ENDHLSL
        }
